...
def parseTweet(tweet, filename, htags):
	try:
		...
		with open(filename, 'a') as tf:
			tf.write("%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\t%s\n" % (createdat, tid, uid, uname, replytostatus, replytouser, oid, ouid, ufollowcount, ufriendcount, ufavecount, ustatuscount, lat, lng, ','.join(mentions), ','.join(hashtags), ','.join(urls), ','.join(media), timestamp-otimestamp, created_at, oufollowcount, oufriendcount, oufavecount, oustatuscount, olat, olng, text))
